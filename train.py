...
from utils.extract_data import build_scene_graph
...
def calculate_training_loss(data, nnet, l_fn, nnet_type):
    ...
    loss = torch.sum(torch.multiply(torch.sum(loss_1, 1), m_i))/torch.sum(m_i) + torch.mean(loss_2)
    return loss, pred_i
...
if __name__ == "__main__":
    ...
    model = SimpleMPGNN(tv_set.num_node_features, tv_set.num_classes + 1, tv_set.num_edge_features, hyperparams["hidden_layers"])
